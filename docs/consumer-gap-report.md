...
| Glass Card | Product 仍有 `.glass-card`；部分接入改成裸 `material-glass-elevated rounded-surface` 后结构差异明显 | `UiGlassSurface` 三档 material；`UiCard` 的无材质 grouping | 无新的通用 Card variant；Consumer 应在 composition boundary 正确选择现有 Surface，业务 hover/content geometry 留本地 | D. Neoverse Product Layer | 否；不增加 `card` variant |
